...
class QuizManager:
    ...
    def next_question_index(self, direction=1):
        ...
        new_index = (direction + current_question_index )% self.total_questions
        st.session_state["question_index"] = new_index
        return self.get_question_at_index(new_index)
 ...
 if question_bank:
        ...
        with st.container():
            ...
            for index, question in enumerate(question_bank):
                form_key = f"Multiple Choice Question {index}"
            # Format the question and display
                with st.form(form_key):
                ##### YOUR CODE HERE #####
                    index_question = quiz_manager.next_question_index()# Use the get_question_at_index method to set the 0th index
                ##### YOUR CODE HERE #####
